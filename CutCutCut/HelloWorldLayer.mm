...
-(id) init
{
	if( (self=[super init])) {
        ...
        m_motionManager.accelerometerUpdateInterval = .2;
        m_motionManager.gyroUpdateInterval = .2;
...
-(void)endGame
{
    ...
    //CustomMenuItem *label = [CustomMenuItem itemWithTarget:[CCLabelTTF labelWithString:@"RESTART"fontName:@"Helvetica Neue"fontSize:50] selector:@selector(restart)];
    ...
    TouchData * td = [[TouchData alloc] init];
    td->m_score = (double)_cuts;
    
    [m_arrData addObject:td];
    
    [[TouchDB dbInstance] saveTouchData:m_arrData];
...
{
    if([TouchDB dbInstance] == NULL){
       // UIAlertView* alert = [[UIAlertView alloc] initWithTitle:@"Action Error" message:@"You didn't start." delegate:nil cancelButtonTitle:@"OK" otherButtonTitles:nil];
        //[alert show];
        //return;
    }
    ...
    for (UITouch *touch in touches){
        ...
        m_ptTouched = location;
        m_dStartedTime = [[NSDate date] timeIntervalSince1970];
...
- (void)ccTouchesEnded:(NSSet *)touches withEvent:(UIEvent *)event
{
	...
	for( UITouch *touch in touches ) {
        ...
        float fX = m_ptTouched.x - location.x;
        float fY = m_ptTouched.y - location.y;
        td->m_fLength = sqrt(fX * fX + fY * fY);
        
        int nDelta = (int)(([[NSDate date] timeIntervalSince1970] - m_dStartedTime) * 1000.0f);
        td->m_fSpeed = td->m_fLength / nDelta;
        
        if (SYSTEM_VERSION_GREATER_THAN_OR_EQUAL_TO(@"9.0")) {
            if (touch.force > td->m_fPressure) {
                td->m_fPressure = touch.force; // / touch.maximumPossibleForce;
            }
        }
        
        if (SYSTEM_VERSION_GREATER_THAN_OR_EQUAL_TO(@"8.0")){
            if (touch.majorRadius > td->m_fArea) {
                td->m_fArea = touch.majorRadius;
            }
...
{
    TouchData * td = [[TouchData alloc] init];
    td->m_fAccelX = acceleration.x;
    td->m_fAccelY = acceleration.y;
    td->m_fAccelZ = acceleration.z;
    [m_arrData addObject: td];
}
...
{
    TouchData * td = [[TouchData alloc] init];
    td->m_fRotateX = rotation.x;
    td->m_fRotateY = rotation.y;
    td->m_fRotateZ = rotation.z;
    [m_arrData addObject:td];
}
